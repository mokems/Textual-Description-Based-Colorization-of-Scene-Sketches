...
def encode_feat_with_text(visual_encoded, vocab_indices, input_e_dims, vocab_size_):
    ...
    with tf.variable_scope("TextLSTM"):
        for i in range(batch_size):
            ...
            rnn_cell_w = tf.nn.rnn_cell.GRUCell(rnn_size)
            cell_w = tf.nn.rnn_cell.MultiRNNCell([rnn_cell_w] * num_rnn_layers, state_is_tuple=False)
            rnn_cell_a = tf.nn.rnn_cell.GRUCell(mlp_dim)
            ...
            state_w_shape[0] = 1  # batch_size -> 1s
            ...
            # def ACM(features):
            #     with tf.variable_scope("ACM"):
            #         features = tf.transpose(features, [0, 3, 1, 2])
            #         weights = conv2d(features, 1, 3, False)
            #         weights = tf.sigmoid(weights)
            #         bias = conv2d(features, 1536, 3, False)
            #         features = weights * features + bias
            #     return features
...
def image_encoder_residual(x, num_residual_units, num_classes, reuse=False, data_format='NCHW', labels=None,
                           scope_name=None):
    """
    :param x: [batch_size, 3, H, W]
    :return:
